...
use VuFind\OnlinePayment\Handler\Connector\TurkuPaymentAPI\Client;
use VuFind\OnlinePayment\Handler\Connector\TurkuPaymentAPI\Item;
use VuFind\OnlinePayment\Handler\Connector\TurkuPaymentAPI\PaymentRequest;
use VuFind\OnlinePayment\Handler\Connector\TurkuPaymentAPI\TurkuSignature;
...
class TurkuPaymentAPI extends AbstractBase
{
    ...
    public function startPayment(
        ...
        if (
            ...
            || isset($this->config->serviceFeeProductCode)
            ...
            if ($serviceFee) {
                $code = $this->config->serviceFeeProductCode ?? $productCode;
                $item = (new Item())
                    ->setSapProduct($sapProduct)
                    ->setDescription(
    ...
    public function processPaymentResponse(
        ...
        if ($transaction->getPaymentIdentifier() !== $params['checkout-stamp']) {
            return [self::PAYMENT_FAILURE, false];
        }
        ...
        switch ($status) {
            case 'ok':
                if ($marked = $transaction->isInProgress()) {
                    $transaction->setPaid();
                    $this->paymentService->persistEntity($transaction);
                }
                $this->addPaymentEvent($transaction, 'Transaction marked as paid');
                return [self::PAYMENT_SUCCESS, $marked];
            case 'fail':
                $transaction->setCanceled();
                $this->paymentService->persistEntity($transaction);
                $this->addPaymentEvent($transaction, 'Transaction marked as canceled');
                return [self::PAYMENT_CANCEL, false];
            case 'new':
            case 'pending':
            case 'delayed':
                $this->addPaymentEvent($transaction, 'Transaction pending (received status $status)');
                return [self::PAYMENT_PENDING, false];
        }

        $this->logPaymentError("unknown status $status");
        $this->addPaymentEvent($transaction, 'Received unknown status', ['status' => $status]);
        return [self::PAYMENT_FAILURE, false];
    }
